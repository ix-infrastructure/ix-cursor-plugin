...
import { wrapText } from "./parser.js";
import { redactSecrets } from "../shared/secrets.js";
export const MIN_LLM_VERSION = [0, 7, 0];
...
async function detectIxVersion() {
...
export function resetLlmSupportCache() {
    supportPromise = null;
}
export async function ixSupportsLlm() {
    if (llmDisabled())
        return false;
    if (!supportPromise) {
...
export function redactLlmText(text) {
...
export function isLlmErrorLine(text) {
...
export async function tryLlm(toolName, args, input, opts = {}) {
    if (!(await ixSupportsLlm()))
        return null;
    let result;
    try {
