...
function extractGrepPattern(command) {
...
async function main() {
    ...
    if (intent === "literal" || intent === "file" || intent === "unknown")
        process.exit(0);
    // Health gate
    ...
    if (IX_HOOK_VERBOSITY === "silent")
        process.exit(0);
    const parts = [`[ix] bash grep intercepted for '${pattern}'`];
    if (locatePart)
    ...
    const agentMessage = IX_HOOK_VERBOSITY === "verbose"
        ...
        : context;
    process.stdout.write(JSON.stringify({ permission: "allow", agent_message: agentMessage }));
    process.exit(0);
}
