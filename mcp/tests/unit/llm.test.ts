...
test("llm text redaction matches the JSON path field-for-field", () => {
  for (const secret of SECRETS) {
    ...
    const jsonField = parseIxJson(JSON.stringify({ token: secret })) as { token: string };
...
test("ixSupportsLlm honors the IX_DISABLE_LLM_FORMAT kill switch", async () => {
  ...
  try {
    ...
    if (prev === undefined) delete process.env["IX_DISABLE_LLM_FORMAT"];
    else process.env["IX_DISABLE_LLM_FORMAT"] = prev;
    resetLlmSupportCache();
  }
});
