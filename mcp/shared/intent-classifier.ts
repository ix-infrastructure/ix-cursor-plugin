...
function isLiteral(p: string): boolean {
  ...
  if (/[*+?]|[[\]()]|\\[a-zA-Z]|\{[0-9]/.test(p)) return true;
...
function asFile(p: string): IntentResult | null {
  // Absolute or relative path-like: starts with / or ./ or contains /
  if (/^(\/|\.\/|\.\.\/)/.test(p)) return { intent: "file", confidence: 0.95 };
...
function asUnknown(p: string): IntentResult | null {
  // Starts with a shell command word
  if (/^(grep|rg|find|ls|cat|git|npm|yarn|cd|echo|curl|wget)\s/.test(p))
    return { intent: "unknown", confidence: 0.3 };
  // Starts with a flag
  if (/^-/.test(p)) return { intent: "unknown", confidence: 0.3 };
  return null;
}

// ── Symbol heuristics ─────────────────────────────────────────────────────────

function asSymbol(p: string): IntentResult {
...
export function classifyIntent(pattern: string): IntentResult {
  const p = pattern.trim();
  if (!p) return { intent: "unknown", confidence: 0 };

  if (isLiteral(p)) return { intent: "literal", confidence: 0.9 };
...
export function looksLikeSecret(pattern: string): boolean {
