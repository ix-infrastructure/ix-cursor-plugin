...
function rateKey(filePath: string): string {
...
async function isRateLimited(filePath: string): Promise<boolean> {
...
async function recordIngest(filePath: string): Promise<void> {
  const key = rateKey(filePath);
  try {
...
function toRelPath(filePath: string, workspaceRoots: string[]): string {
  const root =
...
async function main(): Promise<void> {
  ...
  if (await isRateLimited(filePath)) process.exit(0);
  ...
  const child = spawn(
    "sh",
    ["-c", `${IX_BIN} map "$1" 2>/dev/null || ${IX_BIN} map "$1" 2>/dev/null`, "--", relPath],
