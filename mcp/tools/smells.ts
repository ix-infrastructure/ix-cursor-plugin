...
async function runSmells(input: SmellsInput): Promise<ToolResult> {
  ...
  const raw = parseIxJson(result.stdout) as SmellsRaw;
  ...
  return wrapOk(
    ...
    summarizeSmells(candidates.length, raw.count ?? allCandidates.length),
    undefined,
    result.durationMs,
  );
}

function summarizeSmells(returned: number, total: number): string {
  if (total === 0) {
