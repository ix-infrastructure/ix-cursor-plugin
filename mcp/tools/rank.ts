...
async function runRank(input: RankInput): Promise<ToolResult> {
  ...
  const raw = parseIxJson(result.stdout) as RankRaw;
  ...
  return wrapOk(
    ...
    summarizeRank(by, kind, normalized.length),
    undefined,
    result.durationMs,
  );
}

function summarizeRank(by: string, kind: string, count: number): string {
  if (count === 0) {
